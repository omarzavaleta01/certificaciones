...
list($campus, $contadorCampus) = obtenerCampusEspecifico($conn, $token);
foreach ($campus as $camp) {
    $nombre = $camp["nombre"];
    $noCampus = $camp["numeroCampus"];
    $idEntidad = $camp["idEntidad"];
    $idInstitucionCarrera = $camp["idInstitucion"];
}
...
<div class="row inicial">
    <div class="col-lg-12">
        <hr>
        <a href="index.php"><i class="fas fa-house-user"></i> Inicio</a> > Instituciones > Editar de campus
        <hr>
    </div>
    <div class="col-lg-12">
        <form method="post" action="procesamiento.php?action=editarCampus&idCampus=<?= $token ?>" onsubmit="return confirm('¿Se encuentra segur@ de modificar el campus?')">
            <h5><i class="fas fa-plus-square"></i> Editar de campus</h5>
    ...
    <div class="col-lg-4">
        ...
        <input type="text" name="nombreCampus" value="<?= $nombre ?>" onkeyup="mayus(this);" maxlength="200" class="form-control" required autofocus>
    </div>
    <div class="col-lg-4">
        <label>* Numero de campus</label>
        <input type="number" value="<?= $noCampus ?>" name="noCampus" id="noCampus" min="1" max="999999999" class="form-control" required>
    </div>
    <div class="col-lg-4">
        <label>* Entidad</label>
        <select name="idEntidad" value="<?= $idEntidad ?>" class="form-control" required="">
            <option value="">Seleccione una entidad</option>
            <?php list($entidades, $contadorEntidades) = obtenerCatalogoEntidades($conn);
            foreach ($entidades as $entidad) : ?>

                <?php if ($entidad["idEntidad"] == $idEntidad) : ?>
